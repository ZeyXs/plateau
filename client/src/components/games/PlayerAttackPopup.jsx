            ...
            <div className="flex flex-col space-y-10">
              ...
              <div className="flex flex-row space-x-5 items-center justify-center">
                {Object.keys(players).map((player, index) => (
                  player !== auth.id && (
                    <div key={index} onClick={() => handleSelect(player)} className={`relative flex flex-col items-center justify-center p-2 rounded-lg space-y-2 cursor-pointer ${playersData[player].malus.includes(cardValue) || playersData[player].score == 0 && "border-gray-500 border-2 bg-gray-500 bg-opacity-35 hover:bg-opacity-25"} ${(selectedPlayer === player) && !playersData[player].malus.includes(cardValue) ? "border-red-500 border-2 bg-red-500 bg-opacity-35 hover:bg-opacity-25" : "hover:bg-white hover:bg-opacity-25"}`}>
