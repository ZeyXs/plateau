const GameType = ({ type, title, isSelected, handleGameType, bgUrl }) => {
    return (
        <div onClick={() => handleGameType(type)} className={`rounded-lg z-10 border-spacing-0 border-2 ${isSelected ? 'border-purple-500' : 'border-white'} relative overflow-hidden cursor-pointer`}>
